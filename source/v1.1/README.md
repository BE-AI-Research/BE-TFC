...
6. **Permanent actions ask first.** The Recycle Bin is not *dangerous* — it
   threatens nothing about the system — but emptying it destroys files the
   client chose to keep, and the shell empty can't be quarantined or rolled
   back. It's off by default and asks for consent before it's armed, from every
   route that can arm it. Silent mode can't prompt, so it logs the warning
   instead; naming the id on the command line is the opt-in.
...
**Orphaned-quarantine prompt.** If a quarantine folder has no usable journal —
left by a crash, a killed process, or an older build — there is no record of
where its files came from, so it cannot be rolled back. BE-TFC surfaces it
anyway, separately from the rollbackable count, and offers to discard it and
reclaim the space. The alternative is worse: without this it would sit on the
client's disk indefinitely with nothing ever mentioning it. Silent mode reports
these and clears them only with `--discard-orphans`.

Because quarantined files keep the ACL they had at their source, clearing an
orphaned store generally requires elevation — which BE-TFC always has.
...
| `--discard-orphans` | Delete quarantine folders with no usable journal (left by a crash or an older build). They can't be rolled back; without this flag they're only reported. |
...
Note this returns the **build** version, not the release version. BE-TFC carries
two independent numbers: the release version (what the tool is called — shown in
the title bar, the log header and `--version`) and the build version (assembly
identity, used for `dist\` filenames, checksums and the publish-time assertion).
`--json` reports both, as `version` and `buildVersion`.
...
tests/BETFC.Tests/    xUnit (56 tests: transaction lifecycle, CLI parsing,
                      self-protection bounds, catalog invariants)
